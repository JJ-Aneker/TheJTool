...
// Descarga un documento de referencia desde Supabase Storage como base64
...

{
  ...
  "meta": {
    ...
    // 1. Documento de referencia desde Supabase
    if (refDoc) {
      userContent.push({
        type: 'document',
        source: {
      ...
      userContent.push({
        type: 'text',
        text: `El documento anterior es el DOCUMENTO DE REFERENCIA (${refDoc.filename}) para la vertical ${verticalKey}. Analízalo con detalle — es el modelo exacto de calidad, estructura y redacción que debes replicar en el nuevo documento.`
      })
    }
    ...
    if (files && files.length > 0) {
      ...
      for (const file of files) {
        if (file.type === 'application/pdf') {
          ...
          userContent.push({
            type: 'text',
            text: `[DOCUMENTO: ${file.name}]\n${file.textContent || '(contenido binario)'}\n`
          })
        }
      }
    }
